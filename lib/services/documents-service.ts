...
import createAnalyzeCarImageFunction from "../lambda/analyzeCarImageFunction";

export interface DocumentServiceStackProps {
  readonly claimsBus: EventBus;
}

export class DocumentServiceStack extends Construct {
  readonly documentsBucket: Bucket;
  documentProcessingSM: StateMachine;
  analyzeCarImageFunction: NodejsFunction;

  constructor(scope: Construct, id: string, props?: DocumentServiceStackProps) {
    super(scope, id);
  ...
  private createDocumentProcessingStateMachine(
    props: DocumentServiceStackProps | undefined
  ) {
    ...
    const textractResponseTransformerLambda = new NodejsFunction(
      ...
      "TextractResponseTransformerLambda",
      {
        ...
        handler: "handler",
        entry: "app/handlers/documents/textractResponseTransformer.js",
    ...
    this.analyzeCarImageFunction = createAnalyzeCarImageFunction(this, {
