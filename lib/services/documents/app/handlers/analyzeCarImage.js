...
exports.handler = async function (event) {
  ...
  if (!process.env.COLOR_DETECT_API || !process.env.DAMAGE_DETECT_API) {
    console.error("Color or Damage detection API endpoint has not been specified under lib/config.ts");
  }
...
async function analyzeForDamage(buffer, analyzedFieldAndValues) {
  ...
  const getDamageResp = await callAPI(process.env.DAMAGE_DETECT_API, getDamageAPIOptions, buffer);
...
async function analyzeForColor(buffer, analyzedFieldAndValues) {
  ...
  const getColorResp = await callAPI(process.env.COLOR_DETECT_API, getColorAPIOptions, buffer);
