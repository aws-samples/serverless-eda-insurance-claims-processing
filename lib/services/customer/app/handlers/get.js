...
exports.handler = async function (event, context) {
  ...
  // Option 2: From custom header (voice agent flow) — only trust from the AgentCore service role
  else if (event.headers?.['X-Cognito-Identity-Id']) {
    const callerArn = event.requestContext?.identity?.userArn || '';
    const allowedRolePattern = process.env.ALLOWED_HEADER_ROLE_ARN;
    if (allowedRolePattern && callerArn.includes(allowedRolePattern)) {
      cognitoIdentityId = event.headers['X-Cognito-Identity-Id'];
    } else {
      return {
        statusCode: 403,
        body: JSON.stringify({ error: "Caller not authorized to use identity header" }),
        headers: {
