...
async function checkIdentityFraud({customerId, analyzedFieldAndValues, documentType}) {
  let fraud = {},
    putEventsCommand;
...
async function checkInsuredAssetFraud({customerId, recordId, analyzedFieldAndValues, documentType}) {
  let fraudReason;

  if (analyzedFieldAndValues && analyzedFieldAndValues.type === "claims") {
    fraudReason = "No damage detected.";
    fraudReason = await checkClaimsFraud(customerId, recordId, analyzedFieldAndValues, fraudReason);
...
async function checkClaimsFraud(customerId, claimId, {damage, color}, fraudReason) {
